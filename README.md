...
- **录入**：右下角「+」填写物品名称、地址（可输新地址）、类别、首次更换日期、周期（月 / 年），实时计算下次到期；并可在弹窗内勾选「创建日程提醒」（默认勾选，取消则仅应用内显示、不写入日历）。
- **详情**：点卡片进入，查看更换历史；可「记录本次更换」追加日期，或删除某条历史。
- **到期提醒（写入系统日历）**：开启提醒的物品，临近更换时写入**系统日历日程**（不是闹钟 / 系统通知），由日历在当天提醒；可设提前 7 / 15 / 30 / 60 天，设置入口在物品详情页的「日程提醒」下方。
- **统计**：总数 / 即将到期 / 已逾期 + 未来 6 个月到期分布。
- **备件库存**（底部第 3 个 Tab「备件」）：增删改备件，字段为名称 / 存放位置（地址）/ 数量 / 备注；卡片上直接 +/− 调数量，按存放位置筛选。
...
## 数据存储
- Room 数据库，三张表：`items`（含 `calendarEventId`、`remindEnabled` 字段）、`replacements`、`spare_parts`（独立备件表）。
- 数据库当前版本为 v6；`remindEnabled` 由迁移脚本 `MIGRATION_5_6`（新增列，默认 1）平滑升级，已有数据保留；同时保留 `fallbackToDestructiveMigration()` 兜底。
